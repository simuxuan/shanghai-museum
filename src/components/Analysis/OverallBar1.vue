<template>
  <div class="com-container">
    <div class="com-chart" ref="overall_bar1_ref"></div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    initChart() {
      this.chartInstance = this.$echarts.init(this.$refs.overall_bar1_ref);
      const initOption = {
        tooltip: {
          trigger: 'axis',
          axisPointer: {
          ...
          formatter: function (params) {
            var tar = params[1]
            var ele = params[0]
            return tar.name + '<br/>' + '温度：' + ele.value + '℃' + '~' + (ele.value + tar.value) + '℃'
          }
        },
        grid: {
          width: 300,
